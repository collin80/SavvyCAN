...
ISOTP_HANDLER::ISOTP_HANDLER(const QVector<CommFrame> &modelFrames,
...
void ISOTP_HANDLER::rapidFrames(const QVector<CommFrame>& pFrames)
{
...
void ISOTP_HANDLER::processFrame(const CommFrame &frame)
{
    ...
    case 1: //first frame of a multi-frame message
        checkNeedFlush(ID);
        msg.setBus(frame.getBus());
        if (frame.payload().length() < CAN_FRAME_SIZE) return; //MUST have all 8 data bytes in this first frame.
        msg.setExtendedFrameFormat( frame.hasExtendedFrameFormat() );
