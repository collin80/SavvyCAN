...
void ISOTP_HANDLER::processFrame(const CANFrame &frame)
{
    ...
    case 1: //first frame of a multi-frame message
        checkNeedFlush(ID);
        msg.bus = frame.bus;
        if (frame.payload().size() < 8) return; //MUST have all 8 data bytes in this first frame.
        msg.setExtendedFrameFormat( frame.hasExtendedFrameFormat() );
        msg.setFrameId(ID);
        msg.setTimeStamp(frame.timeStamp());
        msg.isReceived = frame.isReceived;
    ...
    case 2: //subsequent frames for multi-frame messages
        ...
        ln = pMsg->reportedLength - pMsg->payload().size();
        ...
        if (pMsg->reportedLength <= pMsg->payload().size())
        {
...
void ISOTP_HANDLER::checkNeedFlush(uint64_t ID)
{
    ...
    if (messageBuffer.contains(ID))
    {
        ...
        if (msg->reportedLength <= msg->payload().size())
        {
            qDebug() << "Flushing full frame" << QString::number(msg->frameId(), 16) << "  " << msg->reportedLength << "  " << msg->payload().size();
        ...
        else
        {
            ...
            {
                qDebug() << "Flushing a partial frame " << QString::number(msg->frameId(), 16) << "  " << msg->reportedLength << "  " << msg->payload().size();
                if (msg->reportedLength > 0) emit newISOMessage(*msg);
            }
