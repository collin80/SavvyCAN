...
static QVector<CODE_STRUCT> UDS_FILE_MODEOFOP = {{1, "ADDFILE", "Add file to file system"},
...
void UDS_HANDLER::sendUDSFrame(const UDS_MESSAGE& msg)
{
  ...
  if (msg.bus < 0)
    return;
  if (msg.bus >= CANConManager::getInstance()->getNumBuses())
    return;
  if (msg.service > 0xFF)
    return;
...
QString UDS_HANDLER::getDetailedMessageAnalysis(const UDS_MESSAGE& msg)
{
  ...
  else
  {
      ...
      if ((msg.subFunc % 2) == 1)
      {
        ...
        if (dataLen > 1)
        {
          ...
          for (int j = 2; j < dataLen; j++)
            buildString.append(Utility::formatHexNum(data[j]) + " ");
        }
      }
      else
      {
    ...
    case UDS_SERVICES::READ_BY_ID + 0x40:  // reply
      buildString.append("Reply is non-standard and so no decoding is done. The format is (ID) followed by how ever "
                         "much data that ID returns, followed by more ID/data pairs if applicable.\nPayload: ");
    ...
    case UDS_SERVICES::READ_BY_ADDR:
      ...
      if (dataLen > (dataSize + addrSize))
      {
        ...
        for (int i = 0; i < addrSize; i++)
          buildString.append(QString::number(data[1 + i], 16).toUpper().rightJustified(2, '0'));
        buildString.append("\nSize: 0x");
        for (int i = 0; i < dataSize; i++)
          buildString.append(QString::number(data[1 + i + addrSize], 16).toUpper().rightJustified(2, '0'));
      }
      else
      {
    ...
    case UDS_SERVICES::REQUEST_UPLOAD:
      ...
      if (dataLen > (dataSize + addrSize))
      {
        buildString.append("Address: 0x");
        for (int i = 0; i < addrSize; i++)
          buildString.append(QString::number(data[3 + i], 16).toUpper().rightJustified(2, '0'));
        buildString.append("\nSize: 0x");
        for (int i = 0; i < dataSize; i++)
          buildString.append(QString::number(data[3 + i + addrSize], 16).toUpper().rightJustified(2, '0'));
      }
      else
      {
        buildString.append("Message has insufficient bytes to properly decode address and size!");
    ...
    case UDS_SERVICES::REQUEST_UPLOAD + 0x40:
      ...
      for (int i = 0; i < dataSize; i++)
        buildString.append(QString::number(data[2 + i], 16).toUpper().rightJustified(2, '0'));
      break;
