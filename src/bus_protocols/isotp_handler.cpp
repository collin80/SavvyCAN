...
void ISOTP_HANDLER::sendISOTPFrame(int bus, int ID, QByteArray data)
{
  ...
  if (bus < 0)
    return;
  if (bus >= CANConManager::getInstance()->getNumBuses())
    return;
...
void ISOTP_HANDLER::processFrame(const CANFrame& frame)
{
  ...
  case 0:  // single frame message
    ...
    if (frameLen == 0)
      return;  // length of zero isn't valid.
    if (frameLen > 6 && useExtendedAddressing)
      return;  // impossible
    if (frameLen > 7)
      return;
  ...
  case 1:  // first frame of a multi-frame message
    ...
    if (frame.payload().count() < 8)
      return;  // MUST have all 8 data bytes in this first frame.
    msg.setExtendedFrameFormat(frame.hasExtendedFrameFormat());
    msg.setFrameId(ID);
    msg.setTimeStamp(frame.timeStamp());
    msg.isReceived = frame.isReceived;
    ...
    if (useExtendedAddressing)
    {
      ...
      msg.reportedLength = frameLen;
      for (int j = 0; j < 5; j++)
        dataBytes.append(frame.payload()[3 + j]);
    }
    else
    {
      frameLen += data[1];
      ...
      msg.reportedLength = frameLen;
      for (int j = 0; j < 6; j++)
        dataBytes.append(frame.payload()[2 + j]);
    }
    msg.lastSequence = -1;
    msg.setPayload(dataBytes);
  ...
  case 2:  // subsequent frames for multi-frame messages
    ...
    if (!pMsg)
      return;
    if (!pMsg->isMultiframe)
      return;  // if we didn't get a frame type 1 (start of multiframe) first then ignore this frame.
    dataBytes.clear();
    ...
    if (useExtendedAddressing)
    {
      if (ln > 6)
        ln = 6;
      for (int j = 0; j < ln; j++)
        dataBytes.append(frame.payload()[j + 2]);
    }
    else
    {
      if (ln > 7)
        ln = 7;
      for (int j = 0; j < ln; j++)
        dataBytes.append(frame.payload()[j + 1]);
    }
  ...
  case 3:              // flow control messages
    ...
    case 0:  // continue to send frames but maybe change inter-frame delay
      ...
      if (framesUntilFlow == 0)
        framesUntilFlow = -1;  //-1 means don't count frames and just keep going
      // data[2] contains the interframe delay to use (0xF1 through 0xF9 are special through - 100 to 900us)
      if (data[2] < 0xF1)
        frameTimer.start(data[2]);  // set proper delay between frames
      else
        frameTimer.start(1);  // can't do sub-millisecond sending with this code so just use 1ms timing
      break;
...
void ISOTP_HANDLER::checkNeedFlush(uint64_t ID)
{
  ...
  if (messageBuffer.contains(ID))
  {
    ...
    {
      qDebug() << "Flushing full frame" << QString::number(msg->frameId(), 16) << "  " << msg->reportedLength << "  "
               << msg->payload().count();
      if (msg->reportedLength > 0)
        emit newISOMessage(*msg);
    }
    else
    {
      ...
      {
        qDebug() << "Flushing a partial frame " << QString::number(msg->frameId(), 16) << "  " << msg->reportedLength
                 << "  " << msg->payload().count();
        if (msg->reportedLength > 0)
          emit newISOMessage(*msg);
      }
      else
        qDebug() << "Have a partial message but sending of such is disabled. Throwing it away";
    }
...
void ISOTP_HANDLER::frameTimerTick()
{
  ...
  else  // while waiting for a flow frame we didn't get one during timeout period. Try to send anyway with default
        // timeout
  {
    waitingForFlow = false;
...
void ISOTP_HANDLER::removeFilter(int pBusId, uint32_t ID, uint32_t mask)
{
  for (int i = 0; i < filters.count(); i++)
  {
    if (filters[i].bus == pBusId && filters[i].ID == ID && filters[i].mask == mask)
      filters.removeAt(i);
  }
}
