...
{
  if (!gotFrameFunction.isCallable())
    return;  // nothing to do if we can't even call the function
  ...
  for (int i = 0; i < filters.length(); i++)
  {
    if (filters[i].checkFilter(frame.frameId(), frame.bus))
    {
      ...
      for (int j = 0; j < dataLen; j++)
        dataBytes.setProperty(j, QJSValue(data[j]));
...
void ISOTPScriptHelper::newISOMessage(ISOTP_MESSAGE msg)
{
  ...
  for (int j = 0; j < msg.payload().length(); j++)
    dataBytes.setProperty(static_cast<quint32>(j), QJSValue((unsigned char)msg.payload()[j]));
  args.append(dataBytes);
  gotFrameFunction.call(args);
}
...
void UDSScriptHelper::sendUDS(QJSValue bus, QJSValue id, QJSValue service, QJSValue sublen, QJSValue subFunc,
