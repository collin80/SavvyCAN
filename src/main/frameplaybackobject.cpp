...
quint64 FramePlaybackObject::updatePosition(bool forward)
{
  ...
  if (forward)
  {
    if (currentPosition < (currentSeqItem->data.count() - 1))
      currentPosition++;  // still in same file so keep going
...
quint64 FramePlaybackObject::peekPosition(bool forward)
{
  ...
  if (forward)
  {
    if (peekCurrentPosition < (currentSeqItem->data.count() - 1))
...
void FramePlaybackObject::startPlaybackForward()
{
  ...
  if (mThread_p && (mThread_p != QThread::currentThread()))
  {
    QMetaObject::invokeMethod(this, "startPlaybackForward", Qt::BlockingQueuedConnection);
    return;
  }
...
void FramePlaybackObject::startPlaybackBackward()
{
  ...
  if (mThread_p && (mThread_p != QThread::currentThread()))
  {
    QMetaObject::invokeMethod(this, "startPlaybackBackward", Qt::BlockingQueuedConnection);
    return;
  }
...
void FramePlaybackObject::stepPlaybackForward()
{
  ...
  if (mThread_p && (mThread_p != QThread::currentThread()))
  {
    QMetaObject::invokeMethod(this, "stepPlaybackForward", Qt::BlockingQueuedConnection);
    return;
  }
...
void FramePlaybackObject::stepPlaybackBackward()
{
  ...
  if (mThread_p && (mThread_p != QThread::currentThread()))
  {
    QMetaObject::invokeMethod(this, "stepPlaybackBackward", Qt::BlockingQueuedConnection);
    return;
  }
...
void FramePlaybackObject::pausePlayback()
{
  ...
  if (mThread_p && (mThread_p != QThread::currentThread()))
  {
    QMetaObject::invokeMethod(this, "pausePlayback", Qt::BlockingQueuedConnection);
    return;
  }
...
void FramePlaybackObject::setPlaybackInterval(int interval)
{
  ...
  if (mThread_p && (mThread_p != QThread::currentThread()))
  {
    QMetaObject::invokeMethod(this, "setPlaybackInterval", Qt::BlockingQueuedConnection, Q_ARG(int, interval));
    return;
  }
...
void FramePlaybackObject::timerTriggered()
{
  ...
  if (useOrigTiming)
  {
    // get elapsed microseconds since last tick (in case timer skips or is otherwise inaccurate, though there are no
    // guarantees about elapsed timer either)
