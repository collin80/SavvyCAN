...
void FrameSenderWindow::loadSenderFile(QString filename)
{
  ...
  while (ui->tableSender->rowCount() > 0)
    ui->tableSender->removeRow(0);
  sendingData.clear();
  ...
  while (!inFile->atEnd())
  {
    ...
    if (line.length() > 2)
    {
      ...
      if (tokens[0] == "T")
      {
        ...
        ui->tableSender->item(row, ST_COLS::SENDTAB_COL_EN)->setCheckState(Qt::Unchecked);
      if (tokens.length() >= 9)
      {
        for (int i = 1; i < ST_COLS::SENDTAB_COL_COUNT; i++)
        {
      ...
      inhibitChanged = false;
      for (int k = 0; k < ST_COLS::SENDTAB_COL_COUNT; k++)
        processCellChange(row, k);
    }
  }
  inFile->close();
  delete inFile;
  ...
  if (mutex.tryLock())
  {
    ...
    for (int i = 0; i < sendingData.count(); i++)
    {
      ...
      for (int j = 0; j < sendData->triggers.count(); j++)
      {
        ...
        if (trigger->currCount >= trigger->maxCount)
...
/// <summary>
/// Process a single line from the dataGrid. Right now it seems to not trigger at all after the first adding of the code
/// but that seems to maybe be because whichever field you where just in will show up as nothing to the code.
/// </summary>
...
void FrameSenderWindow::processModifierText(int line)
{
  ...
  // yeah, lots of operations on this one line but it's for a good cause. Removes the convenience English versions of
  // the logical operators and replaces them with the math equivs. Also uppercases and removes all superfluous whitespace
  modString = ui->tableSender->item(line, 8)
                  ->text()
                  .toUpper()
                  .trimmed()
                  .replace("AND", "&")
      ...
      for (int x = 0; x < trigToks.length(); x++)
      {
        ...
        if (tok.left(2) == "ID")
        {
          ...
          if (thisTrigger.milliseconds == -1)
            thisTrigger.milliseconds = 0;  // by default don't count, just send it upon trigger
        ...
        else if (tok.startsWith("SIG["))
        {
          ...
          if (semi > -1)  // have a signal value to match too
          {
            ...
            int end = tok.indexOf(']');
            thisTrigger.sigValueDbl = tok.mid(semi + 1, end - semi - 1).toDouble();
          }
        }
      }
