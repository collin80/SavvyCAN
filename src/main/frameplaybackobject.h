...
/*
  broken out functionality that used to be in frameplaybackwindow. That mixed code for Model/View/Controller into one
  giant class. I wasn't too concerned about that but the problem is that all code within a GUI object runs in GUI
  context and that is most certainly not a feature, especially if you have a multi-core CPU. This object lives in a
  separate thread from the GUI and thus is better scheduled and doesn't block the GUI thread. Really all functionality
  in this program should be broken into a separate thread from GUI if it is prone to running a long time and/or taking
  up a lot of CPU time (unless it really does have to interface with the GUI in some way. All gui touching code must run
  on its thread).
*/
