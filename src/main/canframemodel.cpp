...
int CANFrameModel::rowCount(const QModelIndex& parent) const
{
  ...
  {
    return static_cast<int>(filteredFrames.size());
  }
...
{
  Q_UNUSED(index);
  return static_cast<int>(Column::NUM_COLUMN);
}
    ...
    : QAbstractTableModel(parent)
{
  ...
  // this is pretty wasteful. We're storing all frames twice. It may be better for filteredFrames to be a list of
  // pointers. Pointers take up 8 bytes instead of 56 so this is quite a savings for RAM usage. But, then filteredFrames
  // would work differently from frames above so the two could not be used interchangeably. Still think about what can be
  // done.
...
void CANFrameModel::normalizeTiming()
{
  ...
  {
    filteredFrames[i].setTimeStamp(
        QCanBusFrame::TimeStamp(0, filteredFrames[i].timeStamp().microSeconds() - timeOffset));
  }
  this->endResetModel();
 ...
 * quicksort on the columns and interpret the columns numerically. But, correct or not, this implementation is quite
...
uint64_t CANFrameModel::getCANFrameVal(QVector<CANFrame>* frames, int row, Column col)
{
  ...
  case Column::Data:
    for (int i = 0; i < std::min(frame.payload().length(), static_cast<qsizetype>(8)); i++)
...
void CANFrameModel::sortByColumn(int column)
{
  ...
  if (sortDirAsc)
    qSortCANFrameAsc(&filteredFrames, Column(column), 0, filteredFrames.count() - 1);
  else
    qSortCANFrameDesc(&filteredFrames, Column(column), 0, filteredFrames.count() - 1);
  ...
  if (role == Qt::DisplayRole)
  {
    ...
    case Column::TimeStamp:
      ...
      if (ts.type() == QVariant::Double)
        return QString::number(ts.toDouble(), 'f', 5);  // never scientific notation, 5 decimal places
      if (ts.type() == QVariant::LongLong)
        return QString::number(ts.toLongLong());  // never scientific notion, all digits shown
      if (ts.type() == QVariant::DateTime)
        return ts.toDateTime().toString(timeFormat);  // custom set format for dates and times
    ...
    case Column::Remote:
      if (!overwriteDups)
        return QString::number(thisFrame.frameType() == QCanBusFrame::RemoteRequestFrame);
            ...
            if ((sig->multiplexParent == nullptr) && sig->processAsText(thisFrame, sigString))
            {
              ...
              if (sig->isMultiplexor)
              {
                ...
                tempString.append(sig->processSignalTree(thisFrame));
                     ...
                     overwriteDups)  // wasn't in this exact frame but is in the message. Use cached value
            {
              bool isInteger = false;
              if (sig->valType == UNSIGNED_INT || sig->valType == SIGNED_INT)
                isInteger = true;
              tempString.append(
                  sig->makePrettyOutput(sig->cachedValue.toDouble(), sig->cachedValue.toLongLong(), true, isInteger));
              tempString.append("\n");
            }
          }
        }
...
void CANFrameModel::addFrame(const CANFrame& frame, bool autoRefresh = false)
{
  ...
  if (!overwriteDups)
  {
    ...
    {
      qDebug() << "addFrame failed to append. App is probably going to crash. frames.length(): " << frames.length()
               << " Exception: " << ex.what();
    }
  }
...
void CANFrameModel::addFrames(const CANConnection*, const QVector<CANFrame>& pFrames)
{
  ...
  {
    mutex.lock();
    qDebug() << "Frames count: " << frames.length() << " of " << frames.capacity() << " capacity, removing first "
  ...
  {
    mutex.lock();
    qDebug() << "filteredFrames count: " << filteredFrames.length() << " of " << filteredFrames.capacity()
             << " capacity, removing first " << (int)(filteredFrames.capacity() * 0.05) << " frames";
