...
void TriggerDialog::handleCheckboxes()
{
  ...
  if (ui->cbBus->checkState() == Qt::Unchecked)
    ui->txtBus->setText("");
  if (ui->cbMillis->checkState() == Qt::Unchecked)
    ui->txtDelay->setText("");
  if (ui->cbMaxCount->checkState() == Qt::Unchecked)
    ui->txtMaxCount->setText("");
  if (ui->cbMsgID->checkState() == Qt::Unchecked)
    ui->cmMsgID->setCurrentText("");
  if (ui->cbSignal->checkState() == Qt::Unchecked)
    ui->cmSignal->setCurrentText("");
  if (ui->cbSigValue->checkState() == Qt::Unchecked)
    ui->txtSigValue->setText("");
...
void TriggerDialog::regenerateCurrentListItem()
{
  ...
  if (ui->cbMsgID->isChecked() && ui->cmMsgID->currentText().length() > 0)
  {
    ...
    trig.ID = ui->cmMsgID->currentText().replace("0x", "").split("(")[0].toInt(nullptr, 16);
