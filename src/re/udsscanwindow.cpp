    ...
    , ui(new Ui::UDSScanWindow)
{
  ...
  for (int i = 0; i < 13; i++)
    ui->cbScanType->addItem(SCANTYPE_NAMES[i]);
  ...
  for (int n = 0; n < numBuses; n++)
    ui->cbBuses->addItem(QString::number(n));
...
void UDSScanWindow::adaptiveToggled()
{
  if (inhibitUpdates)
    return;
  if (ui->cbAllowAdaptiveOffset->isChecked())
    ui->spinReplyOffset->setEnabled(false);
  else
    ui->spinReplyOffset->setEnabled(true);
  if (currEditEntry)
    currEditEntry->bAdaptiveOffset = ui->cbAllowAdaptiveOffset->isChecked();
}
...
void UDSScanWindow::setSessType()
{
  if (inhibitUpdates)
    return;
  if (currEditEntry)
    currEditEntry->sessType = ui->cbSessType->currentIndex();
}
...
void UDSScanWindow::setBusToScan()
{
  if (inhibitUpdates)
    return;
  if (currEditEntry)
    currEditEntry->busToScan = ui->cbBuses->currentIndex();
}
...
void UDSScanWindow::setNoReplyVal()
{
  if (inhibitUpdates)
    return;
  if (currEditEntry)
    currEditEntry->bShowNoReplies = ui->ckShowNoReply->isChecked();
}
...
void UDSScanWindow::setMaxDelayVal()
{
  if (inhibitUpdates)
    return;
  if (currEditEntry)
    currEditEntry->maxWaitTime = ui->spinDelay->value();
}
...
void UDSScanWindow::setIncrementVal()
{
  if (inhibitUpdates)
    return;
  if (currEditEntry)
    currEditEntry->subfunctIncrement = ui->spinIncrement->value();
}
...
void UDSScanWindow::setReplyOffset()
{
  if (inhibitUpdates)
    return;
  if (currEditEntry)
    currEditEntry->idOffset = ui->spinReplyOffset->value();
}
...
void UDSScanWindow::changedScanType()
{
  ...
  if (currEditEntry)
    currEditEntry->scanType = (SCAN_TYPE)idx;
...
void UDSScanWindow::checkIDRange()
{
  ...
  if (currEditEntry)
    currEditEntry->startID = ui->spinStartID->value();
  if (currEditEntry)
    currEditEntry->endID = ui->spinEndID->value();
...
void UDSScanWindow::checkServiceRange()
{
  ...
  if (currEditEntry)
    currEditEntry->serviceLower = ui->spinLowerService->value();
  if (currEditEntry)
    currEditEntry->serviceUpper = ui->spinUpperService->value();
}
...
void UDSScanWindow::checkSubFuncRange()
{
  ...
  if (currEditEntry)
    currEditEntry->subfunctLower = ui->spinLowerSubfunc->value();
  if (currEditEntry)
    currEditEntry->subfunctUpper = ui->spinUpperSubfunc->value();
}
...
void UDSScanWindow::setupScan(int idx)
{
  ...
  for (uint32_t id = scanEntries[idx].startID; id <= scanEntries[idx].endID; id++)
  {
    ...
    case ST_READ_DTC:
      ...
      break;
    case ST_SEC_ACCESS:
      for (int typ = 1; typ < 0x42;
           typ = typ + 2)  // try each type of session access. In practice only the first 1-3 are likely to work
      {
    ...
    case ST_READ_ID:
      ...
      for (int subf = scanEntries[idx].subfunctLower; subf <= scanEntries[idx].subfunctUpper;
           subf += scanEntries[idx].subfunctIncrement)
      {
        test.subFunc = subf;
        sendOnBuses(test, scanEntries[idx].busToScan);
      }
    ...
    case ST_CUSTOM:
      ...
      {
        test.service = typ;
        for (int subTyp = scanEntries[idx].subfunctLower; subTyp <= scanEntries[idx].subfunctUpper;
             subTyp += scanEntries[idx].subfunctIncrement)
        {
          test.subFunc = subTyp;
          sendOnBuses(test, scanEntries[idx].busToScan);
        }
...
void UDSScanWindow::gotUDSReply(UDS_MESSAGE msg)
{
  ...
  if (numSending == 0)
    return;
  if (currIdx >= numSending)
    return;
  sentFrame = sendingFrames[currIdx];

  id = msg.frameId();

  qDebug() << "UDS message ID " << QString::number(msg.frameId(), 16)
           << "  service: " << QString::number(msg.service, 16) << " subfunc: " << QString::number(msg.subFunc, 16);
  ...
  if ((id == (uint32_t)(sentFrame.frameId() + offset)) || ui->cbAllowAdaptiveOffset->isChecked())
  {
    ...
    if (serviceShortName.length() < 3)
      serviceShortName = QString::number(sentFrame.service, 16);
...
void UDSScanWindow::setupNodes(uint32_t replyID)
{
  ...
  if (serviceShortName.length() < 3)
    serviceShortName = QString::number(sendingFrames[currIdx].service, 16);
  ...
  {
    if (nodeID->child(i)->text(0) == Utility::formatHexNum(replyID) ||
        ((replyID == 0xDEAD5EA1) && (nodeID->child(i)->text(0) == "NO REPLY")))
    {
  ...
  if (!foundReplyMatch)
  {
    ...
    if (replyID != 0xDEAD5EA1)
      replyItem->setText(0, Utility::formatHexNum(replyID));
    else
      replyItem->setText(0, "NO REPLY");
