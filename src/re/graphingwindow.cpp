    ...
    , ui(new Ui::GraphingWindow)
{
  ...
  ui->graphingView->setInteractions(QCP::iRangeDrag | QCP::iRangeZoom | QCP::iSelectAxes | QCP::iSelectLegend |
  ...
  ui->graphingView->legend->setSelectableParts(
  ...
  connect(ui->graphingView, SIGNAL(plottableDoubleClick(QCPAbstractPlottable*, int, QMouseEvent*)), this,
          SLOT(plottableDoubleClick(QCPAbstractPlottable*, int, QMouseEvent*)));
  connect(ui->graphingView, SIGNAL(plottableClick(QCPAbstractPlottable*, int, QMouseEvent*)), this,
          SLOT(plottableClick(QCPAbstractPlottable*, int, QMouseEvent*)));
  ...
  // connect(ui->graphingView, SIGNAL(titleDoubleClick(QMouseEvent*,QCPTextElement*)), this,
  // SLOT(titleDoubleClick(QMouseEvent*,QCPTextElement*)));
  connect(ui->graphingView, SIGNAL(axisDoubleClick(QCPAxis*, QCPAxis::SelectablePart, QMouseEvent*)), this,
          SLOT(axisDoubleClick(QCPAxis*, QCPAxis::SelectablePart)));
  connect(ui->graphingView, SIGNAL(legendDoubleClick(QCPLegend*, QCPAbstractLegendItem*, QMouseEvent*)), this,
          SLOT(legendDoubleClick(QCPLegend*, QCPAbstractLegendItem*)));
  connect(ui->graphingView, SIGNAL(legendClick(QCPLegend*, QCPAbstractLegendItem*, QMouseEvent*)), this,
          SLOT(legendSingleClick(QCPLegend*, QCPAbstractLegendItem*)));
    ...
    for (int j = 0; j < graphParams.count(); j++)
    {
      ...
      for (int i = modelFrames->count() - numFrames; i < modelFrames->count(); i++)
      {
        ...
        if (graphParams[j].ID == thisFrame.frameId() &&
            ((graphParams[j].bus == -1) || (graphParams[j].bus == thisFrame.bus)))
        {
...
void GraphingWindow::gotCenterTimeID(uint32_t ID, double timestamp)
{
  ...
  if (Utility::timeStyle != TS_SECONDS)
    timestamp *= 1000000.0;  // timestamp is always in seconds when being passed so convert if necessary
...
void GraphingWindow::titleDoubleClick(QMouseEvent* event, QCPTextElement* title)
{
  ...
  /*
bool ok;
QString newTitle = QInputDialog::getText(this, "SavvyCAN Graphing", "New plot title:", QLineEdit::Normal, title->text(),
&ok); if (ok)
{
...
void GraphingWindow::axisDoubleClick(QCPAxis* axis, QCPAxis::SelectablePart part)
{
  ...
  if (part == QCPAxis::spAxisLabel)  // Set an axis label by double clicking on it
  {
    bool ok;
    QString newLabel =
        QInputDialog::getText(this, "SavvyCAN Graphing", "New axis label:", QLineEdit::Normal, axis->label(), &ok);
    if (ok)
    {
...
void GraphingWindow::selectionChanged()
{
  ...
  */
  ...
  if (ui->graphingView->xAxis->selectedParts().testFlag(QCPAxis::spAxis) ||
      ui->graphingView->xAxis->selectedParts().testFlag(QCPAxis::spTickLabels) ||
      ui->graphingView->xAxis2->selectedParts().testFlag(QCPAxis::spAxis) ||
      ui->graphingView->xAxis2->selectedParts().testFlag(QCPAxis::spTickLabels))
  {
  ...
  if (ui->graphingView->yAxis->selectedParts().testFlag(QCPAxis::spAxis) ||
      ui->graphingView->yAxis->selectedParts().testFlag(QCPAxis::spTickLabels) ||
      ui->graphingView->yAxis2->selectedParts().testFlag(QCPAxis::spAxis) ||
      ui->graphingView->yAxis2->selectedParts().testFlag(QCPAxis::spTickLabels))
  {
  ...
  for (int i = 0; i < graphParams.count(); i++)
  {
    for (int j = 0; j < graphParams[i].x.count(); j++)
    {
      if (graphParams[i].x[j] < xminval)
        xminval = graphParams[i].x[j];
      if (graphParams[i].x[j] > xmaxval)
        xmaxval = graphParams[i].x[j];
      if (graphParams[i].y[j] < yminval)
        yminval = graphParams[i].y[j];
      if (graphParams[i].y[j] > ymaxval)
        ymaxval = graphParams[i].y[j];
    }
  }
...
void GraphingWindow::removeAllGraphs()
{
  ...
  confirmDialog = QMessageBox::question(this, "Really?", "Remove all graphs?", QMessageBox::Yes | QMessageBox::No);
...
void GraphingWindow::contextMenuRequest(QPoint pos)
{
  ...
  if (ui->graphingView->legend->selectTest(pos, false) >= 0)  // context menu on legend requested
  {
    ...
    menu->addAction(tr("Move to top center"), this, SLOT(moveLegend()))
    ...
    menu->addAction(tr("Move to bottom right"), this, SLOT(moveLegend()))
        ->setData((int)(Qt::AlignBottom | Qt::AlignRight));
    menu->addAction(tr("Move to bottom left"), this, SLOT(moveLegend()))
...
void GraphingWindow::saveSpreadsheet()
{
  ...
  if (dialog.exec() == QDialog::Accepted)
  {
    ...
    for (auto&& graph : graphParams)
    {
      ...
      maxCount = std::max(static_cast<qsizetype>(maxCount), graph.x.count());
    }
    qDebug() << "xMin: " << xMin;
    qDebug() << "xMax: " << xMax;
  ...
  if (dialog.exec() == QDialog::Accepted)
  {
    ...
    for (iter = graphParams.begin(); iter != graphParams.end(); ++iter)
    {
      ...
      if (iter->intelFormat)
        ...
        outFile.write(QString::number(iter->startBit * -1).toUtf8());
      outFile.putChar(',');
  ...
  if (dialog.exec() == QDialog::Accepted)
  {
    ...
    while (!inFile.atEnd())
    {
      ...
      if (line.length() > 2)
      {
        ...
        // should probably do better at merging all the code that is the same between all these formats instead of
        // duplication...
        if (tokens[0] == "Z")  // very newest format, adds ability to set bus number
        {
          ...
          if (tokens.length() > 20)  // even newer format with extra graph formatting options
          {
            ...
            if (tokens[18] == "Y")
              gp.drawOnlyPoints = true;
            else
              gp.drawOnlyPoints = false;
          ...
          if (tokens.length() > 22)
          {
            ...
            if (msg)
            {
              ...
              qDebug() << "Couldn't find the message by name! " << tokens[21] << "  " << tokens[22];
          }
        ...
        else if (tokens[0] == "X")  // second newest format based around signals
        {
          ...
          if (tokens.length() > 21)
          {
            ...
            if (msg)
            {
              ...
              qDebug() << "Couldn't find the message by name! " << tokens[20] << "  " << tokens[21];
          }
...
void GraphingWindow::appendToGraph(GraphParams& params, CANFrame& frame, QVector<double>& x, QVector<double>& y)
{
  ...
  if (params.strideSoFar >= params.stride)
  {
    ...
    tempVal = Utility::processIntegerSignal(frame.payload(), params.startBit, params.numBits, params.intelFormat,
                                            params.isSigned);  //& params.mask;
    double xVal, yVal;
    if (Utility::timeStyle == TS_SECONDS)
    {
...
void GraphingWindow::createGraph(GraphParams& params, bool createGraphParam)
{
  ...
  if (numEntries < 1)
    numEntries = 1;  // could happen if stride is larger than frame count
  ...
  for (int j = 0; j < numEntries; j++)
  {
    ...
    if (params.associatedSignal)
    {
      ...
      if (!params.associatedSignal->isSignalInMessage(frameCache[k]))
      {
        ...
        qDebug() << "Signal in the frame!";
    }
    tempVal =
        Utility::processIntegerSignal(frameCache[k].payload(), sBit, bits, intelFormat, isSigned);  //& params.mask;
    // qDebug() << tempVal;
  ...
  selDecorator =
      new QCPSelectionDecorator();  // this has to be a pointer as it is freed internally to qcustomplot classes
  ...
  if (params.drawOnlyPoints)
    ui->graphingView->graph()->setLineStyle(QCPGraph::lsNone);  // Draw only the points, no connections, no fills
  else
  {
