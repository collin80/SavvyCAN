...
dc SnifferItem::dataChange(uchar i) const
{
  ...
  if (lastMark)
  {
    ...
    else  // mNotch contained a bit pattern so use masks and do things more complicated
    {
      ...
      if (byt == 0)
        return dc::NO;  // and if result is null then nothing changed (other than maybe notched bits and they don't
                        // count)
...
void SnifferItem::update(const CANFrame& pFrame, quint32 timeSeq, bool mute)
{
  ...
  for (int i = 0; i < 8; i++)
    mMarker.data[i] |= mLast.data[i] ^ mCurrent.data[i];  // XOR causes only changed bits to be 1's
...
void SnifferItem::notch(bool pNotch)
{
  ...
  {
    for (int i = 0; i < 8; i++)
      mNotch[i] |= mLastMarker.data[i];  // add changed bits to notch value
  }
