...
protected:
   ...
   */
  CANConnection(QString pPort, QString pDriver, CANCon::type pType, int pSerialSpeed, int pBusSpeed, bool pCanFd,
                int pDataRate, int pNumBuses, int pQueueLen, bool pUseThread);
...
signals:
  ...
  void deviceInfo(int,
                  int);  // First param = driver version (or version of whatever you want), second param a status byte
  ...
  /**
   * @brief event emitted when the CANCon::status of the connection changes (connected->not_connected or the other way
   * round)
...
public slots:
  ...
  /**
   * @brief Add a new filter for the targetted frames. If a frame matches it will immediately be sent via the
   * targettedFrameReceived signal
   ...
   * @param pBusId - Which bus to bond to. Doesn't have to match the call to addTargettedFrame exactly. You could
   * disconnect just one bus for instance.
   * @param ID - 11 or 29 bit ID to match against
   * @param mask - 11 or 29 bit mask used for filter
   * @param receiver - Pointer to a QObject that wants to receive notification when filter is matched
