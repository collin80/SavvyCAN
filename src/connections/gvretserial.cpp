...
GVRetSerial::GVRetSerial(QString portName, bool useTcp)
    ...
    , mTimer(this)
    , /*NB: set this as parent of timer to manage it from working thread */
    useTcp(useTcp)
{
...
void GVRetSerial::piSetBusSettings(int pBusIdx, CANBus bus)
{
  ...
  if (pBusIdx < 2)
  {
    ...
    sendDebug("Got signal to update bauds. 1: " + QString::number((can0Baud & 0xFFFFFFF)) +
              " 2: " + QString::number((can1Baud & 0xFFFFFFF)));
  ...
  else
  {
    /* update baud rates */
    QByteArray buffer;
    sendDebug("Got signal to update extended bus speeds SWCAN: " + QString::number(swcanBaud) +
              " LIN1: " + QString::number(lin1Baud) + " LIN2: " + QString::number(lin2Baud));
    buffer[0] = (char)0xF1;                // start of a command over serial
...
void GVRetSerial::procRXChar(unsigned char c)
{
      ...
      if (rx_step < buildData.length() + 9)
      {
        ...
        if (rx_step == buildData.length() + 8)  // it's the last data byte so immediately process the frame
        {
          ...
          if (!isCapSuspended())
          {
            ...
            // if (continuousTimeSync) txTimestampBasis = QDateTime::currentMSecsSinceEpoch() - (buildFrame.timestamp /
            // 1000);
          }
        }
      }
  ...
  case BUILD_FD_FRAME:
    ...
    rx_step++;
    break;
  case TIME_SYNC:  // gives a pretty good base guess for the proper timestamp. Can be refined when traffic starts to
                   // flow (if wanted)
    switch (rx_step)
    {
    case 0:
...
{
  if (lastSystemTimeBasis != CANConManager::getInstance()->getTimeBasis())
    rebuildLocalTimeBasis();
  // qDebug() << "Tick!";
  ...
  if (CANCon::CONNECTED == getStatus())
  {
    ...
    if (validationCounter == 0 && doValidation)
    {
      if (serial == nullptr && tcpClient == nullptr)
        return;
      if ((serial && serial->isOpen()) || (tcpClient && tcpClient->isOpen()) ||
          (udpClient && udpClient->isOpen()))  // if it's still false we have a problem...
      {
  ...
  if (doValidation && serial && serial->isOpen())
    sendCommValidation();
  if (doValidation && tcpClient && tcpClient->isOpen())
    sendCommValidation();
  if (doValidation && udpClient && udpClient->isOpen())
    sendCommValidation();
}
