    ...
    , mTimer(this) /*NB: set this as parent of timer to manage it from working thread */
{
  ...
  QString hostIPandPort = portName.mid(portName.indexOf("can://") + 6, portName.length() - portName.indexOf("can://") -
                                                                           7);  // 7 is lenght of 'can://' and ')'
...
QString SocketCANd::decodeFrames(QString data, int busNum)
{
  ...
  if (firstIndex > 0)
  {
    ...
    qDebug() << "Received datagramm that starts with fragment (missing '< frame'), this should only occur on startup, "
                "removing...: "
             << framePartial;
  }
  ...
  QStringList frameParsed =
      (frameStr.remove(QRegularExpression("^<")).remove(QRegularExpression(">$"))).simplified().split(' ');
...
void SocketCANd::procRXData(QString data, int busNum)
{
  ...
  case IDLE:
    ...
    if (data == "< hi >")
    {
      ...
      qInfo() << hostCanIDs[busNum]
              << ": Could not open bus. Host did not greet with "
                 "< hi >"
                 ": "
              << data;
    break;
  case BCM:
    ...
    if (data == "< ok >")
    {
      ...
      qInfo() << hostCanIDs[busNum]
              << ": Could not open bus. Host did not respond with "
                 "< ok >"
                 ": "
              << data;
    break;
  ...
  case RAWMODE:
    ...
    if (unprocessedData[busNum].length() > 128)
    {
      ...
      qDebug() << "busNum: " << busNum << "- " << unprocessedData[busNum].length()
               << " bytes in unprocessedData, something is wrong, clearing...";
      unprocessedData[busNum].clear();
    }
    break;
  case ISOTP:
