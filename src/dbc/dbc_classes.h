...
/*classes to encapsulate data from a DBC file. Really, the stuff of interest
  ...
  Messages are comprised of signals. Nodes, signals, and messages potentially
  have attribute values. All of them can have comments.
*/
...
class DBC_MESSAGE;  // forward reference so that DBC_SIGNAL can compile before we
                    // get to real definition of DBC_MESSAGE
class DBC_SIGNAL;
...
public:  // TODO: this is sloppy. It shouldn't all be public!
  ...
  double min;
  double max;
  DBC_NODE* receiver;  // it is fast to have a pointer but dangerous... Make sure
                       // to walk the whole tree and delete everything so nobody
...
class DBCSignalHandler;  // forward declaration to keep from having to include
                         // dbchandler.h in this file and thus create a loop
