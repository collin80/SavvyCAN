...
bool DBC_SIGNAL::isSignalInMessage(const CANFrame& frame)
{
  if (isMultiplexor && !isMultiplexed)
    return true;  // the root multiplexor is always in the message.
  if (isMultiplexed)
  {
    if (parentMessage->multiplexorSignal != nullptr)
    {
      ...
      {
        int val;
        if (!multiplexParent->processAsInt(frame, val))
          return false;
    ...
    return true;  // if signal isn't multiplexed then it's definitely in the message
}

// Take all the children of this signal and see if they exist in the message. Can be called recursively to descend the
// dependency tree
...
bool DBC_SIGNAL::processAsText(const CANFrame& frame, QString& outString, bool outputName, bool outputUnit)
{
  ...
  else if (valType == SP_FLOAT)
  {
    ...
    endResult = (*((float*)(&result)) * factor) +
                bias;  // look away! This is awful. I don't even know for sure if it works. Should test that.
  }
...
QString DBC_SIGNAL::makePrettyOutput(double floatVal, int64_t intVal, bool outputName, bool isInteger, bool outputUnit)
{
  ...
  if (valList.count() > 0)  // if this is a value list type then look it up and display the proper string
  {
    ...
    if (!foundVal)
      outputString += QString::number(intVal);
    if (outputUnit)
...
bool DBC_SIGNAL::processAsInt(const CANFrame& frame, int32_t& outValue)
{
  ...
  result = static_cast<int32_t>(
      Utility::processIntegerSignal(frame.payload(), startBit, signalSize, intelByteOrder, isSigned));
